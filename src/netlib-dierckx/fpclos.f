...
c  rotation with the rows 1,2,...n10 of matrix a.
          do 240 j=1,n10
            piv = h1(1)
c   GH211017
c           if(piv.ne.0.) go to 214
            if(abs(piv)>epsilon(piv)) go to 214
            do 212 i=1,kk
              h1(i) = h1(i+1)
 212        continue
            h1(kk1) = 0.
...
c  rotation with the rows n10+1,...n7 of matrix a.
 250      do 270 j=1,kk
            ...
            piv = h2(j)
c  GH211017
c           if(piv.eq.0.) go to 270
            if(abs(piv)<epsilon(piv)) go to 270
c  calculate the parameters of the givens transformation.
...
c  at ui.
 285      j = l5
          do 140 i=1,kk1
            ...
            piv = h(i)
c  GH211017
c           if(piv.eq.0.) go to 140
            if(abs(piv)<epsilon(piv)) go to 140
c  calculate the parameters of the givens transformation.
...
c  determine the number of knots nplus we are going to add.
        npl1 = nplus*2
        rn = nplus
c GH211017 Added explicity int conversion
